...
const POSITIONS = [
  { key: "bottom-center", label: "底部居中", x: "center", y: 30 },
  { key: "bottom-left", label: "底部左侧", x: 50, y: 30 },
  { key: "bottom-right", label: "底部右侧", x: "right", y: 30 },
  { key: "top-center", label: "顶部居中", x: "center", y: "top" },
  { key: "top-left", label: "顶部左侧", x: 50, y: "top" },
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        if (mode === "page-number") {
          ...
          let x = pos.x === "center" ? (width - textWidth) / 2 : pos.x === "right" ? width - textWidth - 50 : 50;
          let y = pos.y === "top" ? height - 40 : 30;
          ...
          if (headerText.trim()) {
            ...
            const textWidth = font.widthOfTextAtSize(text, fontSize);
            let x = pos.x === "center" ? (width - textWidth) / 2 : pos.x === "right" ? width - textWidth - 50 : 50;
            page.drawText(text, { x, y: height - 40, size: fontSize, font, color: rgb(r, g, b) });
          }
          if (footerText.trim()) {
            const text = footerText.replace("{page}", String(pageNum)).replace("{total}", String(pages.length));
            const textWidth = font.widthOfTextAtSize(text, fontSize);
            let x = pos.x === "center" ? (width - textWidth) / 2 : pos.x === "right" ? width - textWidth - 50 : 50;
            page.drawText(text, { x, y: 30, size: fontSize, font, color: rgb(r, g, b) });
          }
        }
      }
  ...
  return (
    <div className="flex h-full flex-col p-6 overflow-auto">
      <div className="mb-6 shrink-0">
        ...
        <p className="mt-1 text-sm text-slate-500">为 PDF 添加页码或自定义页眉页脚。</p>
      </div>
